"""
Conversor de personaje (PS2 #AMO0) al formato HD del Budokai 1 (#AWO).

Pipeline:
  1. Extraer geometria PS2 (vertices + triangulos por mesh part).
  2. Aplicar skinning (rig PS2 -> coords locales por hueso).
  3. Convertir vertices al layout HD 01BD del B1 (stride 44, big-endian).
...
def build_vertex_hd(vert, bone_idx, weight):
    """Vertice PS2 + skinning -> layout HD 01BD del B1 (stride 44).

    Layout 01BD del B1 (verificado en el Gero B1 HD, que funciona):
      +00: pos.x +04: pos.y +08: pos.z
      +12: w (-1.0) +16: weight +20: BONE INDEX (u32)
      +24: nrm.x +28: nrm.y +32: nrm.z
      +36: FFFFFFFF +40: uv.x
    (NOTA: el bone va en +20 en el B1, NO en +28 como el B3.)
    """
    vx, vy, vz = vert['pos']
    nx, ny, nz = vert['nrm']
    u, vt = vert['uv']
    return (f32(vx) + f32(vy) + f32(vz) +
            f32(-1.0) + f32(float(weight)) +
