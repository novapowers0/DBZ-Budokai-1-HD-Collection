...
// dbz1_region is a project cvar (src/launcher/settings.cpp), shared storage.
...
bool ApplyRegionMount() {
  ...
  const std::string region = REXCVAR_GET(dbz1_region);
  REXLOG_INFO("ApplyRegionMount: dbz1_region='{}'", region);
  ...
  // already points at the assets directory (C:\...\assets), so the region folder
  // is game_data_root/<region>.
